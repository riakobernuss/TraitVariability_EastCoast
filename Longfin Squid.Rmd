...
scot_LFsquid <- subset(GSDET_CAT_INF_SPE_merge, SPEC.y == "LOLIGO PEALEII")
```

Reduce datasets to have same data.
```{r reduce}
glf_LFsquid_red <- glf_LFsquid[, c("LEN_GLF", "SEX_GLF", "MO_DAY_YR", "TEMP_BOT", "DECSLAT", "DECSLON")]
neus_LFsquid_red <- neus_LFsquid[, c("CATCHSEX", "LAT", "LON", "EST_TOWDATE", "BOTTEMP", "LENGTH", "SCINAME", "COMNAME")]
scot_LFsquid_red<- scot_LFsquid[, c("FLEN", "FSEX", "SDATE", "SLAT", "SLONG", "BOTTOM_TEMPERATURE", "SPEC.y", "COMM")]
```
...
neus_LFsquid_red <- rename(neus_LFsquid_red, "SEX" = "CATCHSEX", "LATITUDE" = "LAT", "LONGITUDE" = "LON", "DATE" = "EST_TOWDATE", "TEMPERATURE" = "BOTTEMP")
...
scot_LFsquid_red <- rename(scot_LFsquid_red, "LENGTH" = "FLEN", "SEX" = "FSEX", "TEMPERATURE" = "BOTTOM_TEMPERATURE", "LATITUDE" = "SLAT", "LONGITUDE" = "SLONG", "DATE" = "SDATE", "SCINAME" = "SPEC.y", "COMNAME" = "COMM")
