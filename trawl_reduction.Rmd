...
shelf_file_list <- Sys.glob(path="RV*.RData") 

for (i in 1:length(shelf_file_list)) {
...
Merge GULF data.
```{r merge GULF}
glf_reduced <- left_join(glf_individ_red, glf_trawl_red, by = "CRUISE_NO") #vector memory exhausted
```

```{r merge GULF with datatable}
glf_individ_red <- as.data.table(glf_individ_red)
glf_trawl_red <- as.data.table(glf_trawl_red)

class(glf_individ_red) #check to see that it worked

glf_reduced <- glf_individ_red[glf_trawl_red, nomatch = 0, on = c("CRUISE_NO", "P_STA_NO")]
