...
Reduce Scotian data.
```{r reduce Scotian}
gsdet_reduced <- GSDET[, c("MISSION", "SPEC", "FSHNO", "FLEN", "FSEX", "FMAT", "FWT")]
gsinf_reduced <- GSINF[, c("MISSION", "SDATE", "TIME", "STRAT", "SLAT", "SLONG", "AREA", "DIST", "SPEED", "DEPTH", "SURFACE_TEMPERATURE", "BOTTOM_TEMPERATURE", "BOTTOM_SALINITY", "LATITUDE", "LONGITUDE")]
```

Rename columns so merge is possible.
```{r rename Scotian}
names(gsdet_reduced)[names(gsdet_reduced) == "FSHNO"] <- "CODE"
```

Merge Scotian data.
```{r merege by CODE}
scot_full <- left_join(gsdet_reduced, GSSPECIES, by = "CODE")
```

```{r merge Scotian}
scot_recuded <- left_join(scot_full, gsinf_reduced, by = "MISSION")
#not sure if this is what they should be merged by, could be SETNO?
